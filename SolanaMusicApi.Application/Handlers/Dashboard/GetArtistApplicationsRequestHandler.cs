    ...
    public Task<PaginationResponseDto<ArtistApplicationResponseDto>> Handle(GetArtistApplicationsRequest request, CancellationToken cancellationToken)
    {
        ...
        if (!string.IsNullOrEmpty(request.Filter.Query))
        {
            applications = applications.Where(x => 
                x.User.Email!.Contains(request.Filter.Query) || x.User.UserName!.Contains(request.Filter.Query));
        }
        
        var paginated = new DashboardResponsePaginationDto<ArtistApplication>(request.Filter, applications);
